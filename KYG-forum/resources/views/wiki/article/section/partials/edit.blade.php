@foreach($sections as $section)
   <form method="POST" action="{{ route('wiki.article.section.update', ['wiki' => $wiki, 'article' => $article, 'section' => $section]) }}">
      ...
      <input type="number" value="{{ $section->idsection }}" name="idsection" hidden>
      <input type="text" value="{{ old('content', $section->content) }}" name="content">
      <div class="flex items-center gap-4">
         <x-primary-button>{{ __('Update this Article') }}</x-primary-button>
         @if(session('idupdated') == $section->idsection)
            <p
               id="show-update"
               x-data="{ show: true }"
               x-show="show"
